...
function find(birthdays) {
  ...
  for (let index = 0; index < n; index++) {
    ...
    if (!Array.isArray(element)) {
      seenBirth[element] = birthdays[index + 1];
    }
  }
  //   console.log("seenBirth", seenBirth);
  for (const key in seenBirth) {
    if (Object.hasOwnProperty.call(seenBirth, key)) {
      const element = seenBirth[key];
      let runique = true;

      for (const tkey in seenBirth) {
        if (Object.hasOwnProperty.call(seenBirth, tkey)) {
          const telement = seenBirth[tkey];
          if (tkey != key) {
            if (telement[0] == element[0] && telement[1] == element[1]) {
              runique = false;
            }
          }
        }
